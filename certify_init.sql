...
-- DNIC / Cedula Uruguaya usecase table, read by PostgresDataProviderPlugin (see certify-cedula-uruguaya.properties)
CREATE TABLE certify.cedula_uruguaya (
    ci VARCHAR(255) PRIMARY KEY,
    email_for_auth VARCHAR(255),
...
INSERT INTO certify.cedula_uruguaya (ci, email_for_auth, given_name, family_name_1, family_name_2, birth_date, nationality, place_of_birth, document_type, document_number, issue_date, expiry_date, issuing_authority, face) VALUES ('uy-ci-51234567', 'ciudadano.ejemplo@email.com', 'Sofía', 'García', 'Martínez', '1990-07-22', 'Uruguaya', 'Montevideo, Uruguay', 'National ID', '51234567', '2023-05-15', '2033-05-14', 'National Office of Civil Identification', 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAIAQMAAAD+wSzIAAAABlBMVEX///+/v7+jQ3Y5AAAADklEQVQI12P4AIX8EAgALgAD/aNpbtEAAAAASUVORK5CYII');

-- Extracts the "ci" field from the JSON blob that Certify puts in the pre-authorized-code token's
-- "sub" claim (see certify-cedula-uruguaya.properties scope-query-mapping comment for why this must be
-- a plain function call rather than inline CAST/->> - inline JSON operators crash Hibernate's native
-- query parameter parser).
CREATE OR REPLACE FUNCTION certify.extract_ci_from_claims(claims_json text) RETURNS text AS $$
  SELECT (claims_json::json)->>'ci';
